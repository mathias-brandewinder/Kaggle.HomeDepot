    ...
    *)
    ...
    let ``Bayes score`` : FeatureLearner = 
        fun sample ->
            ...
            let simplifiedTrainset = 
                trainset |> Array.map (fun (l,o) -> if l < 2. then ("lo",o.SearchTerm) else ("hi",o.SearchTerm))
            let mostCommon =
                let lo,hi = simplifiedTrainset |> Array.partition (fun (l,_) -> l = "lo")
                let mostcommonIn xs =
                    xs
                    |> Seq.collect (fun (_,o) -> whiteSpaceTokenizer o |> Array.map stem)
                    |> Seq.countBy id
                    |> Seq.sortByDescending snd
                    |> Seq.take 500
                    |> Seq.map fst
                    |> Set.ofSeq
                let commonLo = mostcommonIn lo
                let commonHi = mostcommonIn hi
                let inter = Set.intersect commonLo commonHi
                let union = Set.union commonLo commonHi
                Set.difference union inter
            let predictor = train simplifiedTrainset bayesTokenizer mostCommon

            fun obs -> if predictor (obs.SearchTerm) = "lo" then 1. else 0.
